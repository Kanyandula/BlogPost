...
- **`login_view`** — Modified. Logs the user in only if `user is not None AND user.email_verified is True`. Any other state — wrong password, unknown email, unverified web account (`is_active=False`), or active-but-unverified API-v1 account — renders the existing login template with a generic "Invalid credentials" error and an always-visible "Didn't receive verification email? Resend" link (no enumeration). This is the single canonical place the verification check lives — `AccountAuthenticationForm` keeps doing field-level validation only, not user resolution.

  **Why two conditions** (`user is not None` *and* `email_verified`)? The only configured backend is `CaseInsensitiveModelBackend`, which inherits `user_can_authenticate()` from Django's `ModelBackend` and returns `None` for any `is_active=False` user. That alone gates the web-registration case (web sets `is_active=False` until verification). The second-stage `email_verified` check exists for accounts created via **API v1**, which keeps `is_active=True` for back-compat — those accounts would otherwise pass `authenticate()` and log in despite never verifying. Keeping both checks means the verification gate doesn't silently break if `is_active` semantics change later (e.g., if API v1 is deprecated and unverified accounts become inactive everywhere).
...
The venv lives inside `djangoprojectdir/` (not at `/home/ephraim/` directly). Install + verification steps live in [`docs/superpowers/runbooks/email-verification-purge-cron.md`](../runbooks/email-verification-purge-cron.md).
...
- **Dual check in `login_view` (`user is not None AND user.email_verified`).** Reads like belt-and-suspenders since `CaseInsensitiveModelBackend` already rejects `is_active=False` users at `authenticate()`. Kept anyway because API v1 creates accounts with `is_active=True, email_verified=False` for back-compat — a v1 user attempting to log in via the web would otherwise pass authentication despite never verifying. Dropping the `email_verified` check would couple the gate to "is_active means verified," which is exactly the coupling decision (above) we're avoiding. If API v1 is removed later, the second check becomes redundant but stays harmless.
...
- **Mail transport / ESP selection.** This spec uses Django's stock `EmailMessage` API; the actual transport is configured at the `EMAIL_BACKEND` level and is intentionally separable from feature behavior. Production migration from Gmail SMTP → Postmark via `django-anymail` (which is what makes the DKIM/SPF/DMARC records for `nyasablog.com` actually authenticate the verification mail) is tracked separately in [`docs/superpowers/plans/2026-05-13-postmark-anymail-migration.md`](../plans/2026-05-13-postmark-anymail-migration.md).
- **Bounce / complaint handling.** Postmark's own internal suppression list prevents retries to hard-bounced addresses, so verification mail to a dead inbox just silently fails (the user sees "we sent a link, check your inbox" — same response as the no-enumeration design). Server-side bounce tracking — surfacing "we couldn't reach this email" on next login, or blocking re-registration of known-bouncing addresses — is Task 8 of the Postmark migration plan above (webhook handlers + a bounce flag on `Account`). Not bundled here because it depends on the migration shipping first.
- **Email-template tagging for Postmark dashboard segmentation.** Tagging verification emails with `tags=['email-verification']` and `metadata={'user_id': ...}` so the Postmark Activity feed shows them grouped — Task 3 of the migration plan. Same reason: depends on Anymail being in place.
